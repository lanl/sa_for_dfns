...
#### BIG NOTE: think about whether this should be done on the unique input
####           points or on the full data.

#### BIG NOTE2: I'm gunna do it on the unique inputs.

# Load in the final GP model:
load(file="hetGP_model_final.Rdata") # model
# Load in the full training data:
load(file="seq_design_data_woOutliers.Rdata") # norm_data
# Load in the testing data:
load(file = "compiled_testing_data_woOutliers.Rdata") # testing_data

orig_model = model
model = mleHetGP( X = list(X0 = model$X0, Z0 = model$Z0, mult = model$mult),
...
graph_data = data.frame(bt = c(Z_train, Z_test), data_set = c(rep("train", times = length(Z_train)),
                                                              rep("test", times = length(Z_test))))
...
##################################
## It really looks like the GP is poorly calibrated to the testing data.
## And that the noise is being poorly estimated (although the mean looks fine).
## Let's play around with the tuning parameters and see if we can get a better fit...

# I believe i recalibrate the GP further up in this model.
# save(model, file = "recalibrated_GP_model.Rdata")
