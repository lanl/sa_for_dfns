...
library(reshape2)
library(gridExtra)
setwd("/Users/murph/dfnworks_variability/sobol_indices")
source("MC_sobol_indices.R")

# Grab the final hetGP model:
models_loc = "/Users/murph/dfnworks_variability/gp_analysis"
model = NULL
load(file=paste(models_loc, "/recalibrated_GP_model.Rdata", sep = ""))
...
models_loc = "/Users/murph/dfnworks_variability/gp_analysis"
model = NULL
load(file=paste(models_loc, "/recalibrated_GP_model.Rdata", sep = "")) # model
load(file=paste(models_loc, "/seq_design_data_woOutliers.Rdata", sep = "")) # norm_data
...
# 0.06069751
# Sooo...pretty similar.  Let's do the latter since it should ideally account for things not in the sample?
